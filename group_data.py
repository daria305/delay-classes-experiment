...
def aggregate_results(df):
    ...
    lower_limit, higher_limit = calculate_confidence(df['Tip Pool Size'], n, 0.95)
    final_df = {
...
def filter_outliers(df, filter_threshold):
    ...
    filtered_df = df.apply(filter_condition, args=[max_dict, filter_threshold], axis=1)

    return filtered_df


def filter_condition(row, max_dict, filter_threshold):
